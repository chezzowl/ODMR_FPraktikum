# copy of the used script without some stuff that was hard-coded for our specific dataset
# main difference: the optimization procedure is NOT started after closing the GUI but the funciton is left inside
# if needed
...
@njit()
def optimization(combis, thetaaxis, goal_theta_1, goal_theta_2, phiaxis, goal_phi_1, goal_phi_2, thetaref, phiref,
                 nvrefs):
    """
...
def sliders_on_changed(val):  # update eigenenergy plots
    """
    Defines action for modifying the lines when any slider's value changes.

    :param val: float, the value of the slider that was changed. Not specifically used, since we just need to know that
        A value changed and then pdate the eigenenergy after reading out all sliders.
    """
...
# colors and markers for the plots (defined in that way, it looked good with our data)
# TODO: on the ling run better to make cyclic iterators / color maps for these! (at least for the measurement data)
iter_mark_MT = ['x', '4', '4', 'x']
iter_col_MT = ['purple', 'olivedrab', 'tab:cyan', 'goldenrod']
iter_col_EF = ['red', 'blue', 'green', 'magenta']  # eigenfrequency calculation plots (updated by the sliders)
...
# Adjust the subplots region to leave some space for the controls and maximize the plot area used
# Note: Adjust these according to your measurement.
# This looks good on my screen, you may have to ajust the values or maybe just the figure size on yours!
fig.subplots_adjust(left=0.07, bottom=0.23, right=0.95, top=0.91, wspace=0.18, hspace=0.0)
...
# ------------------- initial values ------------------- #
# Note: You may want to adjust these according to your measurement.
...
# [p1, p2, p3, p4] = np.abs(Bprojection_scalars(B0, SP.theta_ref,
#                                               SP.phi_ref, alpha, beta, gamma) * (np.sqrt(2) / 2) / B0 )
...
# we know there must be 8 from the eigenfrequency equations ... they might, however, overlap!
[phi_line_up_1] = axPhi.plot(phi_ax, phiresult_matrix[:, 0] / (10 ** 9), linewidth=2, color=iter_col_EF[0])
[phi_line_up_2] = axPhi.plot(phi_ax, phiresult_matrix[:, 1] / (10 ** 9), linewidth=2, color=iter_col_EF[1])
[phi_line_up_3] = axPhi.plot(phi_ax, phiresult_matrix[:, 2] / (10 ** 9), linewidth=2, color=iter_col_EF[2])
[phi_line_up_4] = axPhi.plot(phi_ax, phiresult_matrix[:, 3] / (10 ** 9), linewidth=2, color=iter_col_EF[3])
[phi_line_down_1] = axPhi.plot(phi_ax, phiresult_matrix[:, 4] / (10 ** 9), linewidth=2, color=iter_col_EF[0])
[phi_line_down_2] = axPhi.plot(phi_ax, phiresult_matrix[:, 5] / (10 ** 9), linewidth=2, color=iter_col_EF[1])
[phi_line_down_3] = axPhi.plot(phi_ax, phiresult_matrix[:, 6] / (10 ** 9), linewidth=2, color=iter_col_EF[2])
[phi_line_down_4] = axPhi.plot(phi_ax, phiresult_matrix[:, 7] / (10 ** 9), linewidth=2, color=iter_col_EF[3])
...
# we know there must be 8 from the eigenfrequency equations ... they might, however, overlap!
[theta_line_up_1] = axTheta.plot(theta_ax, thetaresult_matrix[:, 0] / (10 ** 9), linewidth=2, color=iter_col_EF[0])
[theta_line_up_2] = axTheta.plot(theta_ax, thetaresult_matrix[:, 1] / (10 ** 9), linewidth=2, color=iter_col_EF[1])
[theta_line_up_3] = axTheta.plot(theta_ax, thetaresult_matrix[:, 2] / (10 ** 9), linewidth=2, color=iter_col_EF[2])
[theta_line_up_4] = axTheta.plot(theta_ax, thetaresult_matrix[:, 3] / (10 ** 9), linewidth=2, color=iter_col_EF[3])
[theta_line_down_1] = axTheta.plot(theta_ax, thetaresult_matrix[:, 4] / (10 ** 9), linewidth=2, color=iter_col_EF[0])
[theta_line_down_2] = axTheta.plot(theta_ax, thetaresult_matrix[:, 5] / (10 ** 9), linewidth=2, color=iter_col_EF[1])
[theta_line_down_3] = axTheta.plot(theta_ax, thetaresult_matrix[:, 6] / (10 ** 9), linewidth=2, color=iter_col_EF[2])
[theta_line_down_4] = axTheta.plot(theta_ax, thetaresult_matrix[:, 7] / (10 ** 9), linewidth=2, color=iter_col_EF[3])
...
with open(SP.phisweep_file, 'rb') as f:
    # load data into matrix with the following structure:
    # [x-axis, measurement 1, errors measurement 1, measurement 2, errors measurement 2, ...]
    # for the following to work properly, please follow the instructions on the proper file structure
    phidata = np.load(f)
    m_phis_xaxis = phidata[0]
    m_phis_measurements = phidata[1:]
    num_phiplots = int(len(m_phis_measurements) / 2)
    print("loaded ", num_phiplots, " graphs from phi sweep ... ")

with open(SP.thetasweep_file, 'rb') as f2:
    # load data into matrix with the following structure:
    # [x-axis, measurement 1, errors measurement 1, measurement 2, errors measurement 2, ...]
    # for the following to work properly, please follow the instructions on the proper file structure
    thetadata = np.load(f2)
    m_thetas_xaxis = thetadata[0]
    m_thetas_measurements = thetadata[1:]
    num_thetaplots = int(len(m_thetas_measurements) / 2)
    print("loaded ", num_thetaplots, " graphs from theta sweep ... ")
# ------------------------------------------------ #

# deg to rad
m_thetas_xaxis = m_thetas_xaxis * (np.pi / 180)
m_phis_xaxis = m_phis_xaxis * (np.pi / 180)
# for the next lines recall: the structure of the array with the loaded measurements is:
# measurement, errorbars, measurement, errorbars, ...
# that is why we take the 2i-th and (2i+1)-st element
for i in range(num_phiplots):
    axPhi.errorbar(m_phis_xaxis, m_phis_measurements[2 * i], yerr=m_phis_measurements[2 * i + 1], ls='none',
                   marker=iter_mark_MT[i], capsize=3, color=iter_col_MT[i])

for i in range(num_thetaplots):
    axTheta.errorbar(m_thetas_xaxis, m_thetas_measurements[2 * i], yerr=m_thetas_measurements[2 * i + 1], ls='none',
                     marker=iter_mark_MT[i], capsize=3, color=iter_col_MT[i])

# --------- add additional axis area and draw sliders there ----------------- #
# --- Euler angles
alpha_slider_ax = fig.add_axes([0.02, 0.125, 0.38, 0.03], facecolor=axis_color)
alpha_slider = Slider(alpha_slider_ax, '$\\alpha$', - np.pi, np.pi, valinit=alpha, valfmt='%1.4f')
beta_slider_ax = fig.add_axes([0.02, 0.075, 0.38, 0.03], facecolor=axis_color)
beta_slider = Slider(beta_slider_ax, '$\\beta$', 0, np.pi, valinit=beta, valfmt='%1.4f')
gamma_slider_ax = fig.add_axes([0.02, 0.025, 0.38, 0.03], facecolor=axis_color)
gamma_slider = Slider(gamma_slider_ax, '$\\gamma$', - np.pi, np.pi, valinit=gamma, valfmt='%1.4f')
# --- D, E, B
d_slider_ax = fig.add_axes([0.55, 0.125, 0.35, 0.03], facecolor=axis_color)
d_slider = Slider(d_slider_ax, 'D $[10^6 \, \mathrm{Hz}]$', (C.D - C.dD) / (1 * 10 ** 6), (C.D + C.dD) / (1 * 10 ** 6),
                  valinit=C.D / (1 * 10 ** 6), valfmt='%1.2f')
e_slider_ax = fig.add_axes([0.55, 0.075, 0.35, 0.03], facecolor=axis_color)
e_slider = Slider(e_slider_ax, 'E $[10^6 \, \mathrm{Hz}]$', (C.E - C.dE) / (1 * 10 ** 6), (C.E + C.dE) / (1 * 10 ** 6),
                  valinit=C.E / (1 * 10 ** 6), valfmt='%1.3f')
B_slider_ax = fig.add_axes([0.55, 0.025, 0.35, 0.03], facecolor=axis_color)
B_slider = Slider(B_slider_ax, 'B [mT]', C.B0 * 10 ** 3, (C.B0 + 2 * C.B0 / 10) * 10 ** 3,
                  valinit=C.B0, valfmt='%1.3f')

# connect sliders to handler(s)
